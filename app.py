"""
个人 AI 助理 — FastAPI 主服务入口（app.py）

阶段 6.2：RAG + Agent Graph + 文件记忆
启动：uv run app.py
历史快照：backups/snapshots/（由 scripts/snapshot.py 管理）
"""
...
# Agent Graph运行入口——P1 的核心：/chat/graph 调这个函数，不再直调 DeepSeek
...
class GraphChatRequest(BaseModel):
    ...
    和 ChatRequest 的区别：
    - 没有 use_rag：RAG 由 Agent 的 intent_route 自动决定是否调 rag_search 工具
    - stream 默认 False：P1 先跑通 JSON；stream=True 走下面的 SSE 生成器
    """
    session_id: str
    question: str
    stream: bool = False
...
class GraphChatResponse(BaseModel):
    """Agent Graph 非流式响应——字段和 ChatResponse 保持一致，前端好复用。"""
    created_at: str
    reply: str
...
async def generate_graph_stream_response(session_id: str, question: str):
    """Agent Graph 的 SSE 生成器（P1 简化版：Graph 跑完后一次性推送全文）。

    为什么不是真流式？
    - Agent Graph 的 result_synthesis 用的是 model.invoke()（同步、非 stream）
    - P1 目标是先跑通「前端 → Graph → MEMORY.md」链路，真流式留 P1.5

    数据流：
      前端 POST /chat/graph {stream:true}
        → 本函数 run_in_executor 调 run_graph()
          → load_context(PG历史+MEMORY.md)
          → intent_route → [general_chat|use_tool]
          → result_synthesis(写PG) → memory_update(写MEMORY.md)
        → SSE 推 {content: 全文, finished:false} + {finished:true}

    注意：result_synthesis 已经 save_message 了，这里不要再写 PG。
    """
    try:
        # Graph.invoke 是阻塞调用（等 LLM 全跑完），放线程池避免卡死 FastAPI 事件循环
        loop = asyncio.get_event_loop()
        future = loop.run_in_executor(None, run_graph, session_id, question)
        # 把 fn(arg1, arg2) 扔到线程池里跑，别在主线程上阻塞
        reply = await future  # 我（async 函数）先休眠，等 future 跑完了把结果给我

        # 推一条「伪流式」正文事件——格式和普通 /chat SSE 一致，前端不用改解析逻辑
        event = {
            "content": reply,
            "finished": False,
            "created_at": datetime.datetime.now().isoformat(),
...
async def chat_graph(req: GraphChatRequest):
    """
    POST /chat/graph：走 LangGraph Agent 全链路（含记忆更新）。
    
    和 /chat 的分工：
    - /chat        → 直调 DeepSeek 或 RagService（旧路径，RAG toggle 仍用它）
    - /chat/graph  → Agent Graph（意图路由 + 工具 + 记忆）
    
    消息持久化：由 agent/nodes/result_synthesis.py 负责，本端点不再 save_message。
    """
    # 做判断是为了一个函数端点支持两种响应模式
    if req.stream:
        return StreamingResponse(
            generate_graph_stream_response(req.session_id, req.question),   # 生成器函数
            media_type="text/event-stream",
            headers={
                "Cache-Control": "no-cache",       # 禁止浏览器/代理缓存 SSE 流
                "Connection": "keep-alive",         # 保持 TCP 连接不关闭
    ...
    # 非流式：直接调 run_graph，返回 JSON
    reply = run_graph(req.session_id, req.question)
