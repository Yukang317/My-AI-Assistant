...
2. 如果工具检索结果为空，请直接回复用户"知识库中没有检索到与该问题相关的内容。"
3. 如果没有工具结果，以友好的方式直接回复用户
4. 回答应简洁、准确、用中文
5. 不确定时坦诚告知，反问用户获取更多信息
6. 不暴露内部实现细节（如"我调用了RAG搜索"）
...
def result_synthesis(state: MainState, model: BaseChatModel) -> dict:
    ...
    # 3. 在 SystemMessage 中追加历史消息（兼容 dict 和 LangChain 消息类型）
    for msg in history:
        # add_messages reducer 会把 dict 转成 LangChain 对象，所以两种类型都要处理
